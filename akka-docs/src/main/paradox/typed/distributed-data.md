...
When we start up the actor we subscribe it to changes for our key, this means that whenever the replicator sees a change
for the counter our actor will get a @scala[`Replicator.Changed[GCounter]`]@java[`Replicator.Changed<GCounter>`], since
this is not a message in our protocol, we use message transformation function to wrap it in the internal `InternalChanged`
